...
(task-options!
  pom {:project     'cumulo/teakits
       :version     +version+
       :description "teakits"
       :url         "https://github.com/mvc-works/teakits"
       :scm         {:url "https://github.com/mvc-works/teakits"}
...
(deftask dev []
  ...
  (comp
    ...
    (watch)
    (transform-cirru)
    (reload :on-jsload 'teakits.main/on-jsload)
    (cljs)
    (target)))
...
(deftask rsync []
  (with-pre-wrap fileset
    (sh "rsync" "-r" "target/" "tiye:repo/Cumulo/teakits" "--exclude" "main.out" "--delete")
    fileset))
