...
- Defaulted missing/non-map sessions to `schema/session` before the client projection reads fields.
- Added server-tagged regressions for valid/non-map path traversal and missing-session projection.
- Aligned Actions with the current default-Snapshot CLI form and run all definition tests under the server entry so server-only modules are available.

## 中文摘要

- legacy path 在中间值不是 Map 时安全返回 `nil`。
- client projection 在 session 缺失时使用 `schema/session` 默认值。
